    ...
    <motion.div variants={itemVariants} className="flex items-center gap-3 rounded-xl px-3 py-2.5 hover:bg-muted/40 transition-colors group cursor-pointer">
      <div className="min-w-0 flex-1">
        ...
        <p className={cn('text-xs', overdue ? 'text-rose-400' : today ? 'text-amber-400' : 'text-muted-foreground')}>
          {overdue ? '⚠ Просрочено · ' : today ? '📅 Сегодня · ' : ''}{formatDate(task.deadline)}
        </p>
      </div>
...
export default function DashboardPage() {
  ...
  const chartData = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'].map((day, i) => ({
    day,
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {isManager ? `Управление ${total} задачами среди ${users.length} сотрудников` : `У вас ${pending} ожидающих задач`}
          </p>
        </div>
        ...
        {isManager ? (
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <StatCard title="Всего задач" value={total} icon={CheckSquare} color="violet" delay={0} />
            <StatCard title="Завершено" value={completed} icon={TrendingUp} color="emerald" delay={0.05} />
            <StatCard title="В процессе" value={inProgress} icon={Clock} color="sky" delay={0.1} />
            <StatCard title="Сотрудники" value={users.length} icon={Users} color="amber" delay={0.15} />
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <StatCard title="Мои задачи" value={total} icon={CheckSquare} color="violet" delay={0} />
            <StatCard title="Завершено" value={completed} icon={TrendingUp} color="emerald" delay={0.05} />
            <StatCard title="В процессе" value={inProgress} icon={Clock} color="sky" delay={0.1} />
            <StatCard title="Мои навыки" value={skills.length} icon={Zap} color="amber" delay={0.15} />
          </div>
        )}
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="font-semibold">Активность задач</h2>
                <p className="text-xs text-muted-foreground">Обзор за неделю</p>
              </div>
              <div className="flex gap-3 text-xs text-muted-foreground">
                <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-violet-400" />Завершено</span>
                <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-sky-400" />В процессе</span>
              </div>
            </div>
          ...
          >
            ...
            <div className="rounded-2xl border border-border bg-card p-6 flex flex-col items-center gap-3">
              <h2 className="self-start font-semibold text-sm">Уровень завершения</h2>
              ...
              <div className="w-full grid grid-cols-3 gap-2 text-center text-xs text-muted-foreground">
                <div><p className="font-semibold text-amber-400">{pending}</p><p>Ждут</p></div>
                <div><p className="font-semibold text-sky-400">{inProgress}</p><p>Активны</p></div>
                <div><p className="font-semibold text-emerald-400">{completed}</p><p>Готовы</p></div>
              </div>
            </div>
            ...
            {skills.length > 0 && (
              <div className="rounded-2xl border border-border bg-card p-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {skills.length > 8 && <span className="text-xs text-muted-foreground">еще +{skills.length - 8}</span>}
                </div>
              </div>
            )}
          </motion.div>
        ...
        {recentTasks.length > 0 && (
          ...
          >
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="font-semibold">Недавние задачи</h2>
            </div>
